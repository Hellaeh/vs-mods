...
public class Core : ModSystem
{
	...
	private static IInventory backpackInv =>
		cApi!.World.Player.InventoryManager.GetOwnInventory(GlobalConstants.backpackInvClassName);
	...
	public override void StartServerSide(ICoreServerAPI api)
	{
		...
		sChannel = api
			.Network.GetChannel(Consts.Channel)
			.SetMessageHandler<BulkMoveItemsPacket>(ServerMoveItems)
			.SetMessageHandler<MaxRadiusPacket>(
				(sender, packet) =>
				{
					...
					foreach (var player in api.World.AllPlayers.Cast<IServerPlayer>())
						sChannel!.SendPacket<MaxRadiusPacket>(
							new() { Payload = SConfig.MaxRadius },
							player
						);
				}
			);
		...
		api.Event.PlayerJoin += player =>
			sChannel.SendPacket(new MaxRadiusPacket() { Payload = SConfig.MaxRadius }, player);
		api.Event.ServerRunPhase(
			EnumServerRunPhase.Shutdown,
			() => api.StoreModConfig(SConfig, Consts.ServerConfigFile)
		);
	}

	public override void StartClientSide(ICoreClientAPI api)
	{
		if ((Favorite = api.ModLoader.GetModSystem<HelFavorite.Core>()) == null)
		...
		api.Input.RegisterHotKey(
			Consts.QuickRefillHotkeyId,
			Lang.Get(ModId + ":quickrefill"),
			GlKeys.Unknown,
			HotkeyType.InventoryHotkeys,
			ctrlPressed: true
		);
		api.Input.RegisterHotKey(
			Consts.QuickStackHotkeyId,
			Lang.Get(ModId + ":quickstack"),
			GlKeys.Unknown,
			HotkeyType.InventoryHotkeys
		);
		...
		cChannel = api
			.Network.GetChannel(Consts.Channel)
			.SetMessageHandler<MaxRadiusPacket>(static packet =>
				(SConfig.MaxRadius, CConfig.Radius) = (packet.Payload, CConfig.Radius)
			)
	...
	private bool QuickRefill(KeyCombination key)
	{
		...
		foreach ((var inv, var favSlots) in favSlotsFromHotbarAndGeneric)
			foreach (var slotId in favSlots)
			{
				...
				if (
					slot == null
					|| slot.Empty
					|| slot.Itemstack.Collectible.MaxStackSize == slot.StackSize
				)
					continue;
			...
			container =>
			{
				...
				for (int souceSlotId = 0; souceSlotId < sourceInv.Count; ++souceSlotId)
				{
					...
					while (sourceSlotRem > 0)
					{
						...
						var payloadByInv =
							destSlot.Inventory == Favorite.Hotbar ? hotbarPayload : backpackPayload;
		...
		if (hotbarPayload.Count > 0)
			cChannel!.SendPacket(
				new BulkMoveItemsPacket()
				{
					Operation = Operation.QuickRefillHotbar,
					Payload = [.. hotbarPayload.Select(kv => (kv.Key, kv.Value))],
				}
			);
		...
		if (backpackPayload.Count > 0)
			cChannel!.SendPacket(
				new BulkMoveItemsPacket()
				{
					...
					Payload = [.. backpackPayload.Select(kv => (kv.Key, kv.Value))],
				}
			);
	...
	private bool QuickStack(KeyCombination key)
	{
		...
		cChannel!.SendPacket(
			new BulkMoveItemsPacket()
			{
				...
				Payload = [.. payload.Select(kv => (kv.Key, kv.Value))],
			}
		);
