...
public static class ChatCommands
{
	public static void Register(ICoreAPI api)
	{
		...
		command
			.WithDescription(
				"Allows you to add/remove containers you are currently looking at to/from whitelist/blacklist respectively. Also a prefix for other subcommands."
			)
			.RequiresPlayer()
	...
	static TextCommandResult MaxRadiusHandler(TextCommandCallingArgs args)
	{
		...
		Core.cApi?.Network.GetChannel(Consts.Channel)
			.SendPacket<MaxRadiusPacket>(new() { Payload = sc.MaxRadius });
	...
	static class Result
	{
		public static TextCommandResult NoContainer = TextCommandResult.Error(
			"No container selected"
		);

		public static TextCommandResult Added(string classname) =>
			TextCommandResult.Success(Format(Op.Added, classname));

		public static TextCommandResult Removed(string classname) =>
			TextCommandResult.Success(Format(Op.Removed, classname));
